...
from app.schemas.users import Email, ImageList, UserEmail, Message
...
async def create_user(user: UserEmail = Body(...)):
    success = await crud_users.create_user(user)
    if not success:
...
async def delete_user(email: str = Query(...)):
    success = await crud_users.delete_user_by_email(email)
    if not success:
...
async def get_problems_reviewNote_list(
    ...
    images, next_offset = await crud_users.get_review_note_images(
...
async def add_problems_review_note(email: Email, problemId: str):
    ...
    result = await crud_users.add_review_note(email.email, problem_key)
...
async def delete_review_problem(email: Email, problemId: str):
    problem_key = "/".join(json.loads(problemId))
    result = await crud_users.delete_review_note(email.email, problem_key)
